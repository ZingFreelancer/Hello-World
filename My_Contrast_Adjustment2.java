...
public class My_Contrast_Adjustment implements PlugInFilter 
{
	...
	public void run(ImageProcessor ip) 
	{
		...
		pixeLow = getLowPercent(histogram, pixelPercent);
		pixelHigh = getHeighPercent(histogram, pixelPercent);
	...
	private static int getLowPercent(int[] innHistogram, int innPixelPercent)
	{
		...
		{
			//Sum pixels in array
			count = count + innHistogram[x];
			//Check if sum is larger than desired %
			if(count >= innPixelPercent)
			{
				//Return x position
				return x;
			}
	...
	private void getHeighPercent(int[] innHistogram, int innPixelpercent)
	{
		int count = 0;
		for(int x = innHistogram.length-1; x > 0; x--)
		{
			count = count + innHistogram[x];
